...
SCHEDULE_FILE = ROOT / "harness" / "runner" / "v081_schedule.json"
PREREG_FILE = ROOT / "harness" / "prereg" / "v08_preregistration.md"
REPORT_FILE = ROOT / "harness" / "reports" / "v081_heterogeneity.json"

EXPECTED_SCHEDULE_SHA256 = "a3c631628ec1a2afd3e8b5a526702a12de854bd1a8bc0445635c4a4c0b053404"

EXPECTED_EXECUTOR_SHA256 = "5589F8C777A594E81BBBFA15D14BF181B141957825DB176EAEA1ABB5DD2D9842"

EXPECTED_PREREG_SHA256 = "2fe7395f31be202c1c49de4bd9164378ac66c4b96fa328f1131aa1eb07ee4bbb"

EXPECTED_RUNS = 240

TASKS = ["T001", "T002", "T003", "T004"]

MODELS = ["deepseek", "openai_gpt5_mini"]
...
def validate_schedule(
    ...
    if not isinstance(
        ...
        raise RuntimeError(
            "SCHEDULE_OBJECT=FAIL"
        )

    if schedule_document.get(
        "experiment_id"
    ) != "PROMPTFORGE_V0.8.1_NO_GEMINI":
        raise RuntimeError(
            "SCHEDULE_EXPERIMENT_ID=FAIL"
        )
    ...
    if int(
        ...
        raise RuntimeError(
            "SCHEDULE_API_CALLS=FAIL"
        )

    rows = schedule_document.get(
        "rows"
    )
    ...
    if observed_positions != list(
        range(240)
    ):
        raise RuntimeError(
            "SCHEDULE_POSITIONS=FAIL"
        )
    ...
    for row in rows:
        ...
        if task_id not in TASKS:
            raise RuntimeError(
                "SCHEDULE_ROW_TASK=FAIL"
            )

        if model_id not in MODELS:
            raise RuntimeError(
                "SCHEDULE_ROW_MODEL=FAIL"
            )
        ...
        if "gemini" in str(
            ...
            raise RuntimeError(
                "SCHEDULE_GEMINI_MODEL=FAIL"
            )

        if arm_id not in ARMS:
            raise RuntimeError(
                "SCHEDULE_ROW_ARM=FAIL"
            )

        if repetition < 1 or repetition > 15:
            raise RuntimeError(
                "SCHEDULE_ROW_REPETITION=FAIL"
            )
    ...
    if observed_keys != expected_keys:
        raise RuntimeError(
            "SCHEDULE_FACTORIAL_COVERAGE=FAIL"
        )

    print(
        "V081_SCHEDULE_VALIDATION=PASS",
        flush=True,
    )
...
def validate_environment() -> None:
    # V0.8.1 environment gate.
    # All experiment hashes are derived from the actual V0.8.1
    # artifacts at validation time. No inherited V0.8 hash literal
    # is used here.
    ...
    actual_executor = sha256_file(
        executor_file
    ).upper()

    actual_preregistration = sha256_file(
        preregistration_file
    ).lower()

    actual_schedule = sha256_file(
        schedule_file
    ).lower()

    if actual_executor != EXPECTED_EXECUTOR_SHA256.upper():
        raise RuntimeError(
            "EXECUTOR_FROZEN_HASH=FAIL "
            + actual_executor
        )

    if actual_preregistration != EXPECTED_PREREG_SHA256.lower():
        raise RuntimeError(
            "PREREGISTRATION_HASH=FAIL "
            + actual_preregistration
        )

    if actual_schedule != EXPECTED_SCHEDULE_SHA256.lower():
        raise RuntimeError(
            "SCHEDULE_HASH=FAIL "
            + actual_schedule
        )
    ...
    if EXPECTED_RUNS != 240:
        raise RuntimeError(
            "V081_EXPECTED_RUNS=FAIL"
        )

    print(
        "V081_ENVIRONMENT=PASS",
        flush=True,
    )
...
def main() -> int:
    ...
    print(
        "PROMPTFORGE V0.8.1 NO-GEMINI RUNNER"
    )
    print(
        "============================================================"
    )
